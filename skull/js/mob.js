...
function mkBoss(x, y, w) {
    ...
    // 히트박스 = scale(1.8) × 내부 좌표 기준으로 정확히 계산
    // 내부 폭 44px × 1.8 = 79, 내부 높이 73px × 1.8 = 131
    if (w === 5 || w === 6) {
        e.w = 140; e.h = 140;
    } else if (w === 10) {
        // 마왕: 배경 전체가 마왕 — 히트박스는 화면 중앙 심장부 (넓게)
        e.w = CW * 0.55;   // 화면 너비의 55%
        e.h = CH * 0.65;   // 화면 높이의 65%
    } else {
        e.w = 79 + w * 1;   // w1=80, w4=83 — 내부폭 44 × scale1.8
        e.h = 131 + w * 2;  // w1=133, w4=139 — 내부높이 73 × scale1.8
    }
