...
function mkEnemy(x, y, w) {
  ...
  // 적 체력 10배 이상 뻥튀기 (기본 최소 150)
  const hp = (15 + w * 12 + (type === "melee" ? 8 : 0)) * 10;
  return {
    ...
    pDir: Math.random() < 0.5 ? 1 : -1, pT: 0, dead: false, kbT: 0, warnT: 0, warnData: null, world: w,
  };
}

function mkBoss(x, y, w) {
  // 보스 체력 10배 뻥튀기 (기본 최소 3500)
  const hp = (200 + w * 150) * 10; 
  return {
...
function takeDmg(d, attacker, unblockable = false) {
  ...
  if (player.parryT > 0 && !unblockable) {
    camShake = 20; hitStop = 15; skillCD = 0; player.parryT = 0; invT = 40;
...
function updateBoss(e) {
  ...
  if (e.hp < e.maxHp * 0.5) e.phase = 2; e.vy = Math.min(e.vy + GRAV, 14);
  const dx = p.x + p.w / 2 - (e.x + e.w / 2), dy = p.y + p.h / 2 - (e.y + e.h / 2);
  if (!e.warnT) e.facing = dx > 0 ? 1 : -1; const spd = e.phase === 1 ? 2.2 : 3.5;
  ...
  if (e.kbT > 0) { e.kbT--; e.vx *= 0.88; } 
    ...
    if (e.warnT <= 0) {
      ...
      if (w === 1) { 
        ...
        else if (e.ap === 1) { const lBox = calcLaser(originX, e.y + 18, 16, wd.facing); lasers.push({ x: lBox.x, y: e.y + 18, w: lBox.w, h: 16, life: 18, maxLife: 18, color: "#aa00ff", dmg: 1, unblockable: true }); camShake = 5; } 
        else { const lBox = calcLaser(originX, e.y - 5, 30, wd.facing); lasers.push({ x: lBox.x, y: e.y - 5, w: lBox.w, h: 30, life: 25, maxLife: 25, color: "#ff0044", dmg: 1, unblockable: true }); camShake = 8; }
      } else if (w === 3) { 
        if (e.ap === 0) {
          const lBox = calcLaser(originX, e.y + 18, 10, wd.facing); lasers.push({ x: lBox.x, y: e.y + 18, w: lBox.w, h: 10, life: 15, maxLife: 15, color: "#ff1111", dmg: 1, unblockable: true });
          ...
          lasers.push({ x: lBox1.x, y: e.y + 5, w: lBox1.w, h: 12, life: 20, maxLife: 20, color: "#ff0000", dmg: 1, unblockable: true });
          lasers.push({ x: lBox2.x, y: e.y + 30, w: lBox2.w, h: 12, life: 20, maxLife: 20, color: "#ff0000", dmg: 1, unblockable: true });
        ...
        else if (e.ap === 1) { const lBox = calcLaser(originX, e.y + 18, 20, wd.facing); lasers.push({ x: lBox.x, y: e.y + 18, w: lBox.w, h: 20, life: 15, maxLife: 15, color: "#ffd700", dmg: 1, unblockable: true }); camShake = 6; } 
        else { for (let i = 0; i < 12; i++) eBullets.push({ x: e.x + 14, y: e.y + 18, vx: Math.cos((i * Math.PI) / 6) * 5, vy: Math.sin((i * Math.PI) / 6) * 5, life: 100, r: 6 }); }
      } else { 
        if (e.ap === 0) { const lBox = calcLaser(originX, e.y + 10, 40, wd.facing); lasers.push({ x: lBox.x, y: e.y + 10, w: lBox.w, h: 40, life: 30, maxLife: 30, color: "#330066", dmg: 1, unblockable: true }); camShake = 15; } 
          ...
          const lBox = calcLaser(originX, e.y + 20, 16, wd.facing); lasers.push({ x: lBox.x, y: e.y + 20, w: lBox.w, h: 16, life: 20, maxLife: 20, color: "#ff00ff", dmg: 1, unblockable: true });
          for (let s = -3; s <= 3; s++) eBullets.push({ x: e.x + 14, y: e.y, vx: Math.cos(wd.ang + s * 0.3) * 6, vy: Math.sin(wd.ang + s * 0.3) * 6, life: 120, r: 4 });
        }
      }
    }
  } else {
    e.mT--; if (e.mT <= 0) { e.mT = e.phase === 1 ? 60 : 30; e.vx = e.facing * spd; if (Math.abs(dx) < 200 && e.onGround) e.vy = -10.5; }
    e.vx *= 0.84; e.sT--;
